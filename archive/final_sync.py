...
# 配置参数 - 使用纯UUID格式
DATABASE_ID = os.getenv('DATABASE_ID')  # [ARCHIVED] Read from env to avoid embedding production identifiers
...
class NotionSync:
    ...
    def __init__(self):
        ...
        self.database_id = DATABASE_ID  # 使用纯UUID
        self.client = None
...
def main():
    """主函数"""
    logger.info("MT5 到 Notion 交易数据同步开始（最终版本）")
    logger.info("=" * 50)
