...
description: 一步创建新的 OpenSpec change，并生成实现前需要的全部产物。适用于用户想快速描述要构建或修复的内容，并直接拿到 proposal、design、specs、tasks。
license: MIT
...
提出一个新的 change，一次性创建 change 并生成全部相关产物。
...
**输入**：用户请求中应包含一个 change 名称（kebab-case），或者包含对其想构建内容的描述。

**步骤**

1. **如果没有明确输入，先询问用户想构建什么**
...
4. **按顺序创建产物，直到达到 apply-ready**
   ...
   a. **对于每个状态为 `ready` 的 artifact（依赖已满足）**：
      ...
      - 将 `context` 和 `rules` 作为写作约束，但不要把它们原样复制进文件
      - 简要显示进度，例如：`Created <artifact-id>`
...
- 提示：`运行 /opsx:apply，或直接让我开始实现 tasks。`
