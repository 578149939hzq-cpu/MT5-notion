---
name: "OPSX: Apply"
description: 根据 OpenSpec change 实施任务（实验性）
category: Workflow
...
**输入**：可以选择性提供一个 change 名称（例如 `/opsx:apply add-auth`）。如果省略，则检查是否能从对话上下文推断；如果含糊或存在歧义，你必须提示用户选择可用 change。
...
3. **获取 apply 指令**
   ...
   **状态处理：**
   - 如果 `state: "blocked"`（缺少必要 artifacts），显示提示，并建议使用 `/opsx:continue`
   - 如果 `state: "all_done"`，表示全部已完成，并建议 archive
   - 否则继续实现
...
所有任务均已完成！你现在可以使用 `/opsx:archive` 归档这个 change。
```
...
这个 prompt 支持“对一个 change 执行动作”的工作流模型：
